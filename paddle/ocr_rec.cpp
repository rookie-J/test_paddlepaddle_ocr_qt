...
void CRNNRecognizer::LoadModel(const std::string &model_dir) {
  ...
  if (this->use_gpu_) {
    ...
    if (this->use_mkldnn_) {
      ...
      config.SetMkldnnCacheCapacity(10);//设置mkldnn针对不同输入shape的cache容量大小
    }
    config.SetCpuMathLibraryNumThreads(this->cpu_math_library_num_threads_);// 设置CPU Math库线程数，CPU核心数支持情况下可加速预测
  }

  config.SwitchUseFeedFetchOps(false);
  // true for multiple input
  config.SwitchSpecifyInputNames(true);//设置是否需要指定输入tensor的name

  config.SwitchIrOptim(true);// 开启计算图分析优化，包括OP融合等

  config.EnableMemoryOptim();// 开启内存/显存复用
  config.DisableGlogInfo();//去除Paddle Inference运行中的log
